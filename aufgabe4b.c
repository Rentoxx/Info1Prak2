...
{ // Prüfen ob es sich um ein valides Jahr handelt
    if (year > 0)
    {
        return 1;
    }
...
{ // Prüfen ob es sich um eine Valide Zahl für einen Monat handel(kleiner gleich 12 und größer als 0)
    if (month > 0 && month <= 12)
    {
        return 1;
    }
...
{ // Prüft ob es sich bei der Zahl um ein Schaltjahr handelt
...
{ // Funktion zur bestimmung des wochentags für ein bestimmtes Datum mithilfe des vorgegebenen Alogrythmus
...
{ // Verschiebung des Wochentags so, dass Montag = 1, Dienstag = 2, ... , Sonntag = 7
    if (WeekDay >= 2)
    {
        return WeekDay - 1;
    }
